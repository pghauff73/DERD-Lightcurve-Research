#!/usr/bin/env python3
"""Import the single frozen Phase-14 acquisition target and merge receipts.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=Path("data/manifests/phase14_acquisition_order.json"),
    )
    ...
    parser.add_argument(
        "--receipt", type=Path,
        default=Path("artifacts/phase14/phase14_source_acquisition_receipt.json"),
    )
    ...
    for wrapped in wrapped_targets:
            ...
            if not verification["valid"]:
                ...
                rows.append(
                    base | verification | {
                        ...
                        "transport": "LOCAL_RIGHTS_REVIEWED_PHASE14_IMPORT",
        ...
        if old is not None and old.get("status") == "VERIFIED_AND_FROZEN":
            ...
            destination = root / target.source_relative_path
            preserved["receipt_history"] = "PRESERVED_FROM_PHASE13_RECEIPT"
            preserved["locally_present"] = destination.is_file()
            if destination.is_file():
                locally_present_verified += 1
            rows.append(preserved)
        else:
            rows.append(base | {"status": "NOT_SELECTED_IN_PHASE14_PACK"})

    receipt = {
        "receipt_id": "DERD-PHASE14-SOURCE-LOCK-RECEIPT-1.0",
        "receipt_profile": "PHASE14-PERIOD-COORDINATE-ROBUSTNESS-SOURCE-LOCK",
        ...
        "pending_count": sum(
            row.get("status") in {"NOT_SELECTED_IN_PHASE14_PACK", "SELECTED_SOURCE_NOT_PRESENT"}
            for row in rows
        ),
